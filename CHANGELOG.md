...
- All maintenance scripts now run in parallel (configurable via MAX_PARALLEL, default 5)
- Significantly faster execution on large host fleets (up to 10x speedup)
- Added elapsed time display and OK/FAIL/Total summary to all maintenance scripts
- Added progress counter showing [current/total] during execution
- Added ServerAliveInterval=30 to prevent SSH timeouts on slow connections
