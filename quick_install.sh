...
# 34. remove_xpad.sh
# ============================================================================
cat > remove_xpad.sh << 'EOF'
#!/bin/bash
set +e
source ./credentials.conf 2>/dev/null || { echo "ERROR: credentials.conf not found!"; exit 1; }
echo "=== Remove Xpad ==="
echo "Removes Xpad sticky notes from all hosts."
echo ""
for host in $(grep -v "^#" hosts.txt | grep -v "^$"); do
    echo "[$host] Removing Xpad..."
    sshpass -p "$SSH_PASS" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 "$SSH_USER"@"$host" \
        'echo '"$SSH_PASS"' | sudo -S bash -c "
        DEBIAN_FRONTEND=noninteractive apt-get remove --purge -y xpad 2>/dev/null || true
        apt-get autoremove -y 2>/dev/null || true
        echo \"Xpad removed\"
    "' 2>&1 && echo "[$host] OK" || echo "[$host] FAILED"
done
...
if [ "$CURRENT" = "$REMOTE_VERSION" ]; then
    ...
    case $update_choice in
        ...
        2)
            ...
            if [ -z "$UPDATE_FILES" ]; then echo -e "${RED}No previous versions found.${NC}"; exit 1; fi
            i=1; declare -a VERSION_FILES
            while IFS= read -r f; do
                FNAME=$(basename "$f")
                FVER=$(echo "$FNAME" | sed 's/update_v\.//;s/update_v//;s/\.sh$//')
                echo -e "  ${YELLOW}${i}.${NC} ${FNAME}"
                VERSION_FILES[$i]="$f"; i=$((i + 1))
            done <<< "$UPDATE_FILES"
            echo ""; read -p "  Select version [1-$((i-1))]: " ver_choice
            if [ -z "$ver_choice" ] || [ "$ver_choice" -lt 1 ] 2>/dev/null || [ "$ver_choice" -ge "$i" ] 2>/dev/null; then echo "Cancelled."; exit 0; fi
            ...
            echo -e "${GREEN}Reverted.${NC}"; exit 0 ;;
        *) echo "Cancelled."; exit 0 ;;
    esac
else
    echo -e "  Update available: ${GREEN}v.${CURRENT}${NC} -> ${YELLOW}v.${REMOTE_VERSION}${NC}"
    echo ""; read -p "  Download and install? (yes/no): " confirm
    if [ "$confirm" != "yes" ]; then echo "Cancelled."; exit 0; fi
fi
...
for f in hosts.txt mac_addresses.txt wallpapers.txt credentials.conf watchdog_hosts.conf; do
    [ -f "$TARGET_DIR/$f" ] && cp "$TARGET_DIR/$f" "/root/${f}.backup"
done
echo ""
...
if [ -z "$FILE_LIST" ]; then echo -e "${RED}Failed to get file list.${NC}"; exit 1; fi

SCRIPTS=$(echo "$FILE_LIST" | grep '"name"' | grep '\.sh"' | sed 's/.*"name": "//;s/".*//')
OTHER_FILES="CHANGELOG.md README.md version.txt"
TOTAL=$(echo "$SCRIPTS" | wc -l); TOTAL=$((TOTAL + 3))
COUNT=0; FAILED=0
...
for file in $SCRIPTS; do
    ...
    if [ -s "$file" ]; then chmod +x "$file"; echo -e "${GREEN}OK${NC}"; else echo -e "${RED}FAILED${NC}"; FAILED=$((FAILED + 1)); fi
done
...
for file in $OTHER_FILES; do
    ...
    echo -ne "  [$COUNT/$TOTAL] $file... "
    curl -sL "${API_BASE}/contents/${file}?ref=${BRANCH}" -H "Accept: application/vnd.github.v3.raw" -o "$file" 2>/dev/null
    if [ -s "$file" ]; then echo -e "${GREEN}OK${NC}"; else echo -e "${RED}FAILED${NC}"; FAILED=$((FAILED + 1)); fi
done

echo ""
for f in hosts.txt mac_addresses.txt wallpapers.txt credentials.conf watchdog_hosts.conf; do
    [ -f "/root/${f}.backup" ] && cp "/root/${f}.backup" "$TARGET_DIR/$f"
done
